...
{
	double a, b, c, p;
	a = sqrt(SQR(fig->value[2] - fig->value[0]) + SQR(fig->value[3] - fig->value[1]));
	b = sqrt(SQR(fig->value[4] - fig->value[2]) + SQR(fig->value[5] - fig->value[3]));
	c = sqrt(SQR(fig->value[0] - fig->value[4]) + SQR(fig->value[1] - fig->value[5]));
	fig->P = a + b + c;
	p = fig->P / 2;
